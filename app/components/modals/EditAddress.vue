...
const { data } = defineProps({ isOpenModal: false, data: null });
...
const selectCountry = (country) => {
	...
	states.value = [];
	fetchStatesForCountry(country.id);
};
...
onMounted(async () => {
	...
	if (data) {
		await fetchStatesForCountry(data?.country_detail?.id);

		formData.full_name.value.value = data?.full_name;
		formData.country.value.value = data?.country_detail?.id;
		selectedCountryName.value = data?.country_detail?.name ?? "";
		formData.state.value.value = data?.state_detail?.id;
		selectedStateName.value = data?.state_detail?.name ?? "";
		formData.city.value.value = data?.city;
		formData.zip.value.value = data?.zip;
