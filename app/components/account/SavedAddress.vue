<template>
	<div class="flex flex-col gap-4">
		...
		<div class="flex items-center justify-between gap-3">
			<div>
				...
				<p class="text-xs text-gray-400 mt-0.5">
					{{ totalCount > 0 ? `${totalCount} address${totalCount > 1 ? 'es' : ''} saved` : 'Your delivery addresses' }}
				</p>
			</div>
			<button
				type="button"
		...
		<div v-if="loading" class="grid grid-cols-1 sm:grid-cols-2 gap-3">
			<div v-for="i in 4" :key="i" class="bg-white border border-gray-100 rounded-2xl p-4 flex flex-col gap-3">
		...
		<!-- Address grid -->
		<div v-else-if="items.length > 0" class="grid grid-cols-1 sm:grid-cols-2 gap-3">
			<div
				v-for="(item, idx) in items"
				:key="'address_' + idx"
				class="bg-white rounded-2xl border transition-all flex flex-col"
				:class="item?.is_default ? 'border-primary/30 bg-primary/[0.02]' : 'border-gray-100'"
			>
				<!-- Card header -->
				<div class="flex items-center justify-between px-4 pt-4 pb-3">
					<div class="flex items-center gap-2">
						...
						<span class="text-xs font-medium text-gray-700">{{ item?.full_name }}</span>
					</div>
				...
				<div class="px-4 pb-3 flex flex-col gap-0.5">
					<div class="flex items-center gap-1.5">
						<Icon name="mdi:phone-outline" size="11" class="text-gray-300 shrink-0" />
						<span class="text-[11px] text-gray-400">{{ item?.phone }}</span>
					</div>
					<div class="flex items-start gap-1.5">
						<Icon name="mdi:home-outline" size="11" class="text-gray-300 shrink-0 mt-px" />
						<span class="text-[11px] text-gray-400 leading-snug">{{ item?.address }}</span>
					</div>
					<div class="flex items-center gap-1.5">
						<Icon name="mdi:map-outline" size="11" class="text-gray-300 shrink-0" />
						<span class="text-[11px] text-gray-400">{{ item?.city_detail?.name }}, Bangladesh</span>
					</div>
				</div>

				<!-- Actions -->
				<div class="mt-auto px-4 py-3 border-t border-gray-50 flex items-center gap-2">
					<button
						type="button"
		...
		<div v-else class="bg-white border border-gray-100 rounded-2xl flex flex-col items-center py-14 text-center px-4">
			...
			<p class="text-xs text-gray-400">Add a delivery address to speed up checkout</p>
			<button
				type="button"
